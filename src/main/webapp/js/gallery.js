...
function jsiBoxInit(){
    ...
    jsiBox.currentImgIndex=0;jsiBox.linkNodesArray=new Array();
};
function jsiBoxDisplayMainImg(){
    ...
    if(jsiBox.linkNodesArray.length>1){
        ...
        jsiBox.infoNode.innerHTML=info}jsiBoxDimMainImage(10);
...
function jsiBoxResize(){
    ...
    if((Math.abs(deltaHeight)>Math.abs(heightResizeStep))||(Math.abs(deltaWidth)>Math.abs(widthResizeStep))){
        ...
        jsiBox.mainImg.src=jsiBox.preloadImg.src;jsiBox.mainImg.style.display='block';
        var imageTitle=(jsiBox.linkNodesArray[jsiBox.currentImgIndex])?jsiBox.linkNodesArray[jsiBox.currentImgIndex].title:'';
        if(imageTitle!=''){
            jsiBox.titleNode.style.display='block';jsiBox.titleNode.innerHTML=imageTitle;
        }
